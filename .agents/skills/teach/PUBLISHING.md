...
A third trap, and the one that does not need a collapsible block to bite. **A heading may not end in a bare `#`.** Python-Markdown reads a run of `#` at the end of an ATX heading as a closing sequence and strips it, without requiring the space in front of it that CommonMark demands, so `# Learning: C#` shipped to the site as "Learning: C" while GitHub kept the character. Escape it, `# Learning: C\#`, which both renderers resolve to a literal `#`, neither shows the backslash, and the heading's anchor is unchanged either way because the slug drops the punctuation. A real closing sequence, with a space before it, means the same thing in both and is left alone.

Do not read for the structural checks, run them: `python3 .agents/tools/check-workspace.py learning/<domain>/<topic>`, which takes one or more workspaces and exits non-zero on any problem. It covers front matter and its keys, the H1 against the front-matter title, the three bold lines, section presence and order, `<details markdown="1">` at column zero with a blank line after `<summary>` and before `</details>`, balanced tags, the closing block byte for byte, dashes, draft markers, machine-specific strings, a heading ending in a bare `#`, relative links, contiguous numbering, README rows against lesson front matter, and glossary ordering. Extend that script rather than writing a new one; where a rule is one workspace's local convention it belongs in its `CONVENTIONS` entry, not in the shared rules.

A track scaffolded from `templates/learning-workspace` but not yet taught has no `lessons/` directory: git does not track empty directories, and `SKILL.md` says to create each lazily on first need. That is a supported, committed state. The checker still runs the prose, link, glossary and `RESOURCES.md` annotation checks against it, and only skips the per-lesson checks and the arc-to-lesson cross-check, which have nothing to check until the first lesson exists. It reports `NO PROBLEMS (0 lessons, scaffold: lessons/ not created yet)` rather than bailing out.
