...
Topics live under [`learning/`](learning/), grouped by domain. Each is a track with a stated mission, an ordered set of short lessons, cheat sheets built for lookup, and a record of what was demonstrably learned. The `teach` skill reads the track at the start of a session, writes one lesson, and records only what the session earned.
...
Run the `teach` skill again. It picks up from the track state, so there is no need to recap what you covered last time.
...
.
├── learning/
│   ├── README.md          # index of topic tracks
│   └── <domain>/<topic>/  # one track per topic
├── templates/
│   └── learning-workspace/ # copy source for a new topic
└── .agents/
    └── skills/teach/      # the format spec every track follows
```
