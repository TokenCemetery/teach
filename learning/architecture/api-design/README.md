...
- Making the compiler reject an impossible state inside one codebase: see [`programming/typescript`](../../programming/typescript/) for that boundary. This workspace is the same instinct applied across a boundary where there is no shared compiler.
