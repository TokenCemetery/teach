...
![A two-column comparison. On the left, what a managed service automates: patching and minor version upgrades, automated backups with point-in-time recovery, and failure detection paired with automated standby promotion. On the right, what still exists and still needs understanding underneath, unchanged: bloat and autovacuum tuning, replication receive and replay lag diagnosis, and every index's write-time maintenance cost. A managed service changes access and tooling, not whether these mechanisms happen.](images/managed-automates-vs-mechanics-unchanged.svg)
