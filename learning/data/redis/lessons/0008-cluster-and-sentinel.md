...
![Two side-by-side diagrams. On the left, without a hash tag, the keys user:123:cart and user:123:profile hash independently and land on two different nodes, Node A and Node C, so a multi-key operation touching both can't be atomic across them. On the right, with the hash tag user:123 applied as curly-brace-user:123-cart and curly-brace-user:123-profile, both keys hash on the same tag and land on the same node, Node B, so an operation touching both stays atomic.](images/cluster-hash-tags.svg)
