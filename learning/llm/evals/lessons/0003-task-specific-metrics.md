...
![Reference "the cat sat on the mat" and prediction "a cat sat on the mat" laid out as token boxes. cat, sat, on, mat and one instance of the are shared between them. The reference's second the and the prediction's a are each unmatched. Five of six tokens match on each side, so precision is 5/6, recall is 5/6, and F1 is 5/6.](images/f1-token-overlap.svg)
