...
![A worked example with y = 3x + 1 and z = y squared, at x = 2. The forward pass, drawn top to bottom left to right, computes x = 2, then y = 7 through the operation y = 3x + 1, then z through the operation z = y squared. The backward pass, drawn below running right to left, starts at z, multiplies by the local derivative dz/dy = 14 to reach y, then multiplies by the local derivative dy/dx = 3 to reach x, giving dz/dx = 14 times 3 = 42 by the chain rule.](images/chain-rule-worked-example.svg)
