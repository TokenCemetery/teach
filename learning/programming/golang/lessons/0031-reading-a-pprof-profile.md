...
Bind it to localhost or put it behind auth on a separate port. `/debug/pprof` exposes your source paths, your goroutine stacks, and a way to make the process do work, so it is not something to serve publicly.
...
It reveals source file paths, function names and full goroutine stacks, which is a map of your internals, and the endpoints let an unauthenticated caller make the process do expensive work on demand.
