...
![The eight default phases in order, with three commands drawn as spans beneath them. All three begin at validate; mvn compile ends at compile, mvn package at package, mvn install at install.](images/up-to-and-including.svg)

Three commands, one left edge. What a command runs is a prefix of the same list every time, so "what will this do" is answered by finding where its phase sits rather than by remembering.
