...
![Three objects drawn to a byte scale. Point with the default header is 12 bytes of header, 8 of fields and 4 of padding. With the compact header it is 8 and 8 with no padding. An int array of five is 16, 20 and 4. Every bar ends on an eight-byte mark.](images/object-layout.svg)

The padding is drawn as an empty box because that is what it is. Reading across, the accented slice is the only part any declaration mentions, and in the first row it is a third of the object. The gridlines are every 8 bytes, so the reason the padding exists is visible rather than asserted: each bar has to reach the next line.
