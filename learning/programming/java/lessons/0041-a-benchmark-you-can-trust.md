...
![Four intervals on one microseconds-per-operation scale. In the underpowered run the precompiled interval sits entirely inside the original's. In the properly configured run the two intervals are separated by a clear gap.](images/read-the-intervals.svg)

Drawn rather than read, the two runs stop looking like the same kind of evidence. In the top pair the shorter interval is contained by the longer one, so every value the precompiled version might really have is a value the original might really have too. In the bottom pair there is daylight between them, and it is that gap, not the distance between the two dots, that the 13 per cent rests on.
