...
![A two by two grid crossing object reference (it vs this) against return value (lambda result vs original object). it plus lambda result is let. it plus original object is also. this plus lambda result is run. this plus original object is apply. with sits outside the grid, since it takes the object as an ordinary argument rather than as a receiver.](images/scope-functions-grid.svg)
