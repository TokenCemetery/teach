...
![Five moments across one mutex. The vector reaches its final value at the push and stays there; the poison flag turns true at the panic and back to false at clear_poison; lock returns Err exactly while the flag is true.](images/the-flag-not-the-data.svg)

The data row is constant from the second column on. Everything that goes wrong afterwards happens in the row above it, which is what "advisory" means here.
