...
![One table's file in four states: 27 MB all in use, 54 MB with 47.08 per cent dead, still 54 MB with that portion now free rather than dead, and 27 MB again after VACUUM FULL.](images/reusable-is-not-returned.svg)

The second and third bars are the same length on purpose, because the file was. Only the name of half of it changed, from dead to free, and a monitoring check that watches file size would have seen the `VACUUM` do nothing at all.
