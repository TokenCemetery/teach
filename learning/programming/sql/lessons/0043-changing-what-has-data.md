...
![Two statements watched from a second session. The NOT VALID statement is a short bar and the second session's select is blocked for exactly its duration. The validate statement is a long bar and the second session's update runs throughout it.](images/heavy-lock-brief-light-lock-long.svg)
